KScens <-
  function(times, cens = rep(1, length(times)),
    ...
    n <- length(times)
    dd <- data.frame(left = as.vector(times),
                     right = ifelse(cens == 1, times, NA))
    alpha <- params$shape
    gamma <- params$shape2
    mu <- params$location
    beta <- params$scale
    ...
    pvalue <- 2 * pnorm(-kr) - 2 * sum((-1)^id * exp(-2 * id^2 * A^2) *
                                         (pnorm(2 * id * A * sr + kr) -
                                            pnorm(2 * id * A * sr - kr)))
  output <- list(
    Test = round(c("p-value" = pvalue, A = A, "F(ym)" = R, ym = stimes[m]),
    degs), Distribution = distr,
    Parameters = round(c(shape = alpha, shape2 = gamma, location = mu,
                         scale = beta), degs))
  if (prnt == TRUE){
    if (outp == "table") {
